...
function NewsContent() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 💡 왓츠앱 캐시를 뚫기 위해 v=${Date.now()} 랜덤값 추가
    ...
    return (
      <div className="min-h-screen bg-white text-black font-sans">
        <div className="bg-red-700 text-white py-3 px-4 flex items-center justify-between font-black italic shadow-lg sticky top-0 z-50">
          ...
          <span className="animate-pulse text-sm flex items-center font-bold">
  ...
  return (
    <main className="min-h-screen bg-[#1a1a1a] flex flex-col items-center justify-center p-4 text-black">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden p-8 border-b-8 border-red-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 text-left">Título de la noticia</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 text-left">Imagen de portada</label>
            <input 
              type="file" 
              name="image" 
              accept="image/*" 
              className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 border-2 p-2 rounded-xl bg-white" 
              required 
            />
          </div>
        ...
        {link && (
          <div className="mt-6 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-xl border-dashed">
            <p className="text-[10px] font-bold text-yellow-800 mb-2 uppercase">✅ Link listo:</p>
