...
function NewsContent() {
  ...
  // 🎣 [낚시 화면] 사용자님의 v/page.tsx 디자인 그대로 적용
  if (t) {
    return (
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden p-8 border-b-8 border-red-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-black">
          <div>
            ...
            <input name="title" placeholder="Ej: 멕시코 축구팀 해체!" className="w-full border-2 p-4 rounded-xl font-bold outline-none focus:border-red-600" required />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Imagen de portada</label>
            {/* 💡 파일 선택 버튼이 확실하게 보이도록 스타일 수정 */}
            <input 
              type="file" 
              name="image" 
              accept="image/*" 
        ...
        {link && (
          <div className="mt-6 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-xl border-dashed">
            <p className="text-[10px] font-bold text-yellow-800 mb-2 uppercase">✅ Link de la broma listo:</p>
            <input readOnly value={link} className="w-full p-2 border mb-3 text-[10px] bg-white" />
